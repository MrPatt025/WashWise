...
export function useAnnouncement() {
  ...
  const announce = React.useCallback((message: string, clearAfter = 1000) => {
...
export function SkipToMain({
  ...
  return (
    <a
      href={href}
      className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[100] focus:rounded-md focus:bg-background focus:px-4 focus:py-2 focus:font-medium focus:text-foreground focus:ring-2 focus:ring-primary"
    >
      {children}
    </a>
  );
...
export const keyboardHelpers = {
  ...
  isActivation: (e: React.KeyboardEvent) => e.key === Keys.Enter || e.key === Keys.Space,
 ...
 */
export function getTabProps(index: number, selectedIndex: number, tabId: string, panelId: string) {
  return {
    tabProps: {
      role: "tab",
      id: tabId,
